...
create_matrix=function(x){
  ...
  temp=matrix(0,length(unique(x$Predator)),length(unique(x$Predator)))
  # Assign names for rows and columns of the 0-full matrix
  colnames(temp)=unique(x$Predator)
  row.names(temp)=unique(x$Predator)
  # Create progress bar
  pb <- txtProgressBar(min = 0, max = length(unique(x$Predator)), style = 3)
  # Start building the matrix
  
  for (j in 1:length(unique(x$Predator))) {	# Loop through each 
    Sys.sleep(0.1)		# Refreshing rate of progress bar
    setTxtProgressBar(pb, j)	# Update progress bar at each iteration
    for (k in 1:length(unique(x$Predator))) {
...
create_matrix_marre=function(x,y,z,Matrix=F,descriptor=T,Count=F, year=F, name_tab = "data"){
  ...
  if(year==F){
    marres=unique(x$Id_Sites) #Assign in an object with the name of the site
    
    
    pb <- txtProgressBar(min = 0, max = length(marres), style = 3) #Create the progress bar
    
    temp_count=matrix(nrow = length(marres),ncol = length(unique(x$Taxon))+1)
    colnames(temp_count)=c(as.character(as.data.frame(table(x[,7]))[,1]),"Site")
    
    descriptors=as.data.frame(matrix(nrow=length(unique(x$Id_Sites)),ncol=18)) #Create an empty data frame for the descriptors
    ...
    name_site=unique(x$Id_Sites) #Assign in an object with the name of the site
    name_matrix=1:length(unique(x$Id_Sites)) #Assign a numeric level for the name of matrix
    ...
    for (o in 1:length(marres)) {#Loop through each ponds
      ...
      temp_count[o,1:length(unique(x$Taxon))]=as.data.frame(table(x[x[,2]==marres[o],7]))[,2]
      temp_count[o,length(unique(x$Taxon))+1]=as.character(name_site[o])
      
      temp_site=x[x[,2]==marres[o],] #Subset the taxon occurrence data for a define pond
      temp_site=droplevels(temp_site) #Forget old unique
      ...
      level=as.character(c(unique(temp_site$Taxon))) #Take species species column name of species present in ponds
      ...
      if(descriptor==T){ # If arg "descriptor" is true, calculate the descriptor
        ...
        descriptors[o,1]=as.character(name_site[o])
    ...
    descriptors$Site=as.factor(as.character(descriptors$Site))
    ...
    col_temp=temp_count[,length(unique(x$Taxon))+1]
    temp_count[temp_count>0]=1
    temp_count[,length(unique(x$Taxon))+1]=col_temp
  ...
  if(year==T){
    ...
    name_site=unique(x$Id_Sites) #Assign in an object with the name of the site
    name_matrix=1:length(unique(x$Id_Sites)) #Assign a numeric level for the name of matrix
    year_matrix=1:length(unique(x$Annee))
    
    for (o in 1:length(marres)) {#Loop through each ponds
      Sys.sleep(0.1) # Define refresh rate for progress bar
      setTxtProgressBar(pb, o) #Set the current status of the progress bar
      
      for(h in 1:length(unique(x$Annee))){
        if(years[h] %in% x[x[,2]==marres[o],3]){
          
          temp_count[o,1:length(unique(x$Taxon))]=as.data.frame(table(x[x[,2]==marres[o] & x[,3]==years[h],7]))[,2]
          temp_count[o,length(unique(x$Taxon))+1]=name_site[o]
          
          temp_site=x[x[,2]==marres[o] & x[,3]==years[h],] #Subset the taxon occurrence data for a define pond
          temp_site[,2]=droplevels(temp_site[,2]) #Forget old unique
          temp_site[,7]=droplevels(temp_site[,7]) #Forget old unique
          
          nm=paste("Matrix_site_",marres[o],"_",years[h],sep="") #Define the name of the matrix returned for the current pond
          
          
          level=as.character(c(unique(temp_site$Taxon)))
          
          
          if(length(level)==1){level=as.character(c(unique(temp_site$Taxon),"Zooplankton","Phytoplankton","Detritus","Macrophyte"))}
...
data_format2=function(x,y,z){
  ...
  x=merge(x,y,by.x = c("Id_Sites"),by.y = c("Mares"))
