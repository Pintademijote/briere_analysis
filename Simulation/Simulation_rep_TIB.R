...
simu_TIB_Rep=function(x,y,t=200,name_tab="data",repeti=100){
  pb <- txtProgressBar(min = 0, max = repeti, style = 3)
  start.time = Sys.time()
  allspecies=colnames(x)
    ...
    for (mares in 1:length(DisP)) {
      ...
      for (i in 2:t) {
        ...
        if(i>1){
          ...
          possible=allspecies[!(allspecies %in% present)]
          ...
          if(Extinction==1 & length(present[!(present %in% Base)])!=0){
            presenti=present[!(present %in% Base)]
            rmv=sample(presenti,1)
            temp[rmv,i]=0
...
#simu_TIB_Rep(Matrix_Reg_Manual,envi_data,t=500,name_tab="data",repeti = 1000)

#hist(data$n_mean, breaks = 50)

#write.table(data,"simulation_TIB.txt", sep="\t")

countstat=1:100
for (i in 1:100) {
  tempcount=simu_TIB_Rep(Matrix_Reg_Manual,envi_data,t=500,name_tab="data",repeti = 1000)
  countstat[i]=sum(tempcount)/156000*100
}
