...
layout.matrix.1[troph$TL==quant[2],1]=seq(1,9,9/7)
layout.matrix.1[troph$TL>quant[2] & troph$TL<quant[3],1]=seq(1,9,9/7)
layout.matrix.1[troph$TL>=quant[3] & troph$TL<quant[4],1]=seq(1,length(troph$TL[troph$TL>=quant[3] & troph$TL<quant[4]]))
layout.matrix.1[troph$TL>=quant[4] & troph$TL<quant[5],1]=seq(1,9,9/8)
...
layout.matrix.1[troph$TL>=quant[7] & troph$TL<quant[8],1]=seq(1,9,9/8)
...
basalnb=which(colnames(Matrix_Reg_Manual) %in% c('Detritus','Macrophyte','Phytoplankton',"Zooplankton"))


tempforcom=sample(1:63,20,replace=F)

tempforcom=c(basalnb,tempforcom)

tempforcom=tempforcom[!duplicated(tempforcom)]

temp_com=Matrix_Reg_Manual[tempforcom,tempforcom]

  
  
g=graph_from_adjacency_matrix(temp_com, mode="undirected")

troph=TrophInd(temp_com)

sumspe=colSums(temp_com)
...
layout.matrix.1[,2]<-troph$TL # y-axis value based on trophic level
layout.matrix.1[,1]<-sample(1:length(tempforcom),length(tempforcom),replace=F)
layout.matrix.1[1,1]=2
layout.matrix.1[2,1]=6
layout.matrix.1[2,1]=9
...
#temp_com_bonne1=temp_com
#temp_com_bonne2=temp_com
...
g=graph_from_adjacency_matrix(temp_com_bonne1, mode="undirected")
enaStructure(temp_com_bonne1)$ns[,1]
enaStructure(temp_com_bonne1)$ns[,2]
enaStructure(temp_com_bonne1)$ns[,3]
modularity(g,membership(cluster_walktrap(g)))
mean(TrophInd(temp_com_bonne1)[,1])
sd(TrophInd(temp_com_bonne1)[,1])
mean(TrophInd(temp_com_bonne1)[,2])
sd(TrophInd(temp_com_bonne1)[,2])

###############


g=graph_from_adjacency_matrix(temp_com_bonne2, mode="undirected")
enaStructure(temp_com_bonne2)$ns[,1]
enaStructure(temp_com_bonne2)$ns[,2]
enaStructure(temp_com_bonne2)$ns[,3]
modularity(g,membership(cluster_walktrap(g)))
mean(TrophInd(temp_com_bonne2)[,1])
sd(TrophInd(temp_com_bonne2)[,1])
mean(TrophInd(temp_com_bonne2)[,2])
sd(TrophInd(temp_com_bonne2)[,2])
