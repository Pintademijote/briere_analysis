...
p5=p5+geom_point(data=data_ALL[data_ALL$Simu==c("TIB", "TTIB"),],aes(x=Distance, y=Mean_trophic_level, shape = Simu), size = 3, alpha=0.8) +
  ...
  p7=p7+geom_point(data=data_ALL[data_ALL$Simu==c("TIB", "TTIB"),],aes(x=Distance, y=Mean_Omnivory_Index,shape = Simu), size = 3, alpha=0.8) +
...
top=ggarrange(p3,p5,p7,p9, ncol=2,nrow=2,font.label=c(size=32), vjust=1)


p1_top=ggarrange(NULL,p1,ncol=3,widths = c(1,2,1),font.label=c(size=32), vjust=1)

Top_top=ggarrange(top,p1_top,nrow =2,heights=c(2,1))
...
bottom=ggarrange(p4,p6,p8,p10,ncol=2,nrow=2,
                 font.label=c(size=32), vjust=1)

p2_top=ggarrange(NULL,p2,ncol=3,widths = c(1,2,1),font.label=c(size=32), vjust=1)

bottom_bottom=ggarrange(bottom,p2_top,nrow =2,heights=c(2,1))
